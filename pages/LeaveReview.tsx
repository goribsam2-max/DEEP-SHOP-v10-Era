...
import { isScamReview } from "../lib/reviewModeration";
...
const LeaveReview: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    const bannedWord = await checkContainsBannedWord(comment);
    if (bannedWord) {
      return notify(`Your review contains prohibited content ("${bannedWord}"). Please edit your review.`, "error");
    }

    if (isScamReview(comment)) {
      // Auto-remove: Drop scam accusations silently as requested without exposing restrictions
      notify("Review submitted successfully! Thank you for your feedback.", "success");
